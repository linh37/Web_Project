...
<div id="wrapper">
  ...
  <div class="container" id="new-product" style="padding: 100px 0 0 0"><!--banner-->
      <div style="padding: 0 0 30px 0"> <!--title-->
      <h2 class="chp-title">
...
<div class="container-fluid"> 
    ...
    <div class="col-md-3 col-xs-12">
      <div id="accordion" class="panel panel-primary behclick-panel">             
        <div class="panel-heading">                    
          <h3 class="panel-title" style="margin-left: 100px; font-size:20px">Giá tiền</h3>             
        </div>             
            ...
            <div id="collapse0" class="panel-collapse collapse in">                       
              <ul class="list-group">                          
                <li class="list-group-item">                                
                  <div class="checkbox"><label><button type="submit" name="1" style="width: 200px"> Dưới 100.000₫ </button>
                  </div>                         
                </li>                           
                <li class="list-group-item">                                
                  <div class="checkbox"><label><button type="submit" name="2" style="width: 200px"> 100.000₫ - 300.000₫ </button>
                  </div>                         
                </li>                           
                <li class="list-group-item">                                
                  <div class="checkbox"><label><button type="submit" name="3" style="width: 200px">300.000đ - 500.000đ</button>
                  </div>
                </li>                           
                <li class="list-group-item">                                
                  <div class="checkbox"><label><button type="submit" name="4" style="width: 200px">Trên 500.000₫</button>                                
                  </div>                         
                </li>                       
              </ul>                    
            </div>
  ...
  if (isset($_GET['1'])) {
    $tv="SELECT product.`id`, catalog.`name`,`product`.`name`, product.`price`, image.`image_link`
        FROM product JOIN image ON product.`id` = image.`product_id` JOIN catalog ON catalog.`id` = product.`catalog_id`
        WHERE  product.`price` < 100000
        limit $so_du_lieu offset $vtbd";
  }else if (isset($_GET['2'])) {
    $tv="SELECT product.`id`, catalog.`name`,`product`.`name`, product.`price`, image.`image_link`
        FROM product JOIN image ON product.`id` = image.`product_id` JOIN catalog ON catalog.`id` = product.`catalog_id`
        WHERE product.`price` >= 100000 AND product.`price` < 300000
        limit $so_du_lieu offset $vtbd";
  }else if (isset($_GET['3'])) {
    $tv="SELECT product.`id`, catalog.`name`,`product`.`name`, product.`price`, image.`image_link`
        FROM product JOIN image ON product.`id` = image.`product_id` JOIN catalog ON catalog.`id` = product.`catalog_id`
        WHERE product.`price` >= 300000 AND product.`price` < 500000
        limit $so_du_lieu offset $vtbd";
